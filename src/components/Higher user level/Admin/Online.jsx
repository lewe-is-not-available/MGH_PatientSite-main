...
const Online = ({ ol }) => {
  ...
  return (
    ...
    >
      ...
      <td className="px-6 py-4 bg-slate-100 group-hover/tr:bg-slate-300 transition duration-75 ease-in">{ol.docname}</td>
      <td className="px-6 py-4">{CreatedAt}</td>
      <td className="px-6 py-4 bg-slate-100 group-hover/tr:bg-slate-300 transition duration-75 ease-in">{ol.date}</td>
      <td className="px-6 py-4">{ol.time}</td>
      <td className="px-6 py-4 bg-slate-100 group-hover/tr:bg-slate-300 transition duration-75 ease-in">{ol.reason}</td>
      <td className="px-6 py-4"> {isMedical ? (
          <>
            {isSomeone && (
              <SomeonDetails
              isSomeone={isSomeone}
            ...
            <button
              onClick={() => {handleSomeoneData(ol.online_id);
              }}
      ...
      <td className="px-6 py-4 bg-slate-100 group-hover/tr:bg-slate-300 transition duration-75 ease-in">
        {isMedical ? (
          <>
            ...
            <button
              onClick={() => {handleMedData(ol.online_id);
              }}
      ...
      <td className="px-6 py-4 w-[15%] text-white">
        <Link
          to={"/Appointment_Details/" + ol.online_id}
