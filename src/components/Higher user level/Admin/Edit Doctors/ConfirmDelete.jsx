...
const ConfirmDelete = ({ data, setConfirmDelete }) => {
  ...
  async function handleSubmit(e) {
    ...
    setLoadConfirm(true);
    //*Delete image
    const { error: storageErr } = await supabase.storage.deleteBucket(
      data.email
    );
    if (storageErr) {
      toast.error(storageErr);
    }
    const { data: DocID, error: fail } = await supabase
      .from("profile")
      .select("id")
      .eq("email", data.email)
      .single();
    ...
    try {
      ...
      if (fail) throw fail;
        ...
        if (delErr) {
          ...
          //*Delete delete entire info
          const { error: drInfoErr } = await supabase
            .from("dr_information")
            .delete()
            .eq("email", data.email);
          try {
            if (drInfoErr) throw drInfoErr;
            else {
            }
          } catch (drInfoErr) {
            setTimeout(() => {
              console.log(drInfoErr);
              toast.error(drInfoErr.message + "49");
              setLoadConfirm(false);
            }, 1000);
          }
          setTimeout(() => {
