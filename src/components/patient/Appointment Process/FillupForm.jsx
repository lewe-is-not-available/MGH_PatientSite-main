...
const OnlineConsult = ({ openTerms, token }) => {
    ...
    if (!token) {
      if (isSomeone) {
        setRepresentative(
          formData.yourFname +
            " " +
            (formData.yourMname ? formData.yourMname + " " : "") +
            formData.yourLname
        );
      }
    }
  ...
  useEffect(() => {
    if (token) {
      setNumber(token.user.user_metadata.phone);
      setRepresentative(
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (!Valid) {
        throw new Error("Please finish the captcha first");
      }
      if (!isSelected) {
        throw new Error("Please finish the payment first");
      } else {
        if (!token) {
        ...
        const { erro: payErr } = await supabase.storage
          .from("images")
          .upload(
            formData.Gmail + "/payment/" + bookID + "/" + uuidv4(),
            File[0]
          );
        if (payErr) {
          throw payErr;
          ...
          <>
            ...
            >
              ...
              <div className=" shadow-lg">
                ...
                {doc.schedule &&
                  doc.schedule.map((sched, i) => (
                    <div
                      key={i}
                      className="flex items-center text-base px-2 py-2 bg-slate-100 justify-between"
                    >
              ...
              <>
                ...
                <div>
                    ...
                    >
                      ...
                      <p>
                        ...
                        <input
                          ...
                          value={number}
                          onChange={(e) =>
                            setNumber(e.target.value.slice(0, 11))
                          }
                          required
                          disabled={token}
                  ...
                  <div className="col-span-3 mb-6">
                    <button
                      type="submit"
                      className="bg-[#418D3F] w-full py-1  font-semibold text-xl text-white rounded-md transition duration-10 ease-in-out 
                        hover:bg-[#A5DD9D] hover:text-[#267124] hover:ring-[#418D3F] hover:ring-[3px]"
                    >
